...
class TaskBuilder(private val constants: Constants, startPose: Pose2d, poseMap: PoseMap) {
     ...
     */
    @JvmOverloads
    fun waitFor(t: Double, unit: Time = Seconds) = apply { waitSeconds(unit.of(t).`in`(Seconds)) }
    ...
    fun afterDisp(ds: Double, unit: Distance = Inches, a: Action) =
        apply { builder = builder.afterDisp(unit.of(ds).`in`(Inches), a) }
    ...
    fun afterTime(dt: Double, unit: Time = Seconds, a: Action) =
        apply { builder = builder.afterTime(unit.of(dt).`in`(Seconds), a) }
    ...
    fun turn(angle: Double, unit: Angle = Radians) =
        apply { builder = builder.turn(unit.of(angle).`in`(Radians), turnConstraints) }
    ...
    fun turnTo(heading: Double, unit: Angle = Radians) =
        apply { builder = builder.turnTo(unit.of(heading).`in`(Radians), turnConstraints) }
    ...
    fun lineToX(posX: Double, unit: Distance = Inches) =
        apply { builder = builder.lineToX(unit.of(posX).`in`(Inches), velConstraints, accelConstraints) }
    ...
    fun lineToXConstantHeading(posX: Double, unit: Distance = Inches) =
        apply {
            builder = builder.lineToXConstantHeading(unit.of(posX).`in`(Inches), velConstraints, accelConstraints)
        }
    ...
    fun lineToY(posY: Double, unit: Distance = Inches) =
        apply { builder = builder.lineToY(unit.of(posY).`in`(Inches), velConstraints, accelConstraints) }
    ...
    fun lineToYConstantHeading(posY: Double, unit: Distance = Inches) =
        apply {
            builder = builder.lineToYConstantHeading(unit.of(posY).`in`(Inches), velConstraints, accelConstraints)
        }
    ...
    fun strafeTo(pos: Vector2d, unit: Distance = Inches) =
        apply {
            builder = builder.strafeTo(
                Vector2d(unit.of(pos.x).`in`(Inches), unit.of(pos.y).`in`(Inches)),
                velConstraints,
                accelConstraints
            )
        }
    ...
    fun strafeToLinearHeading(pos: Vector2d, posUnit: Distance = Inches, heading: Rotation2d) =
        apply {
            builder = builder.strafeToLinearHeading(
                Vector2d(
                    posUnit.of(pos.x).`in`(Inches),
                    posUnit.of(pos.y).`in`(Inches)
        ...
        apply {
            builder = builder.strafeToLinearHeading(
                Vector2d(
                    posUnit.of(pos.x).`in`(Inches),
                    posUnit.of(pos.y).`in`(Inches)
                ), headingUnit.of(heading).`in`(Radians), velConstraints, accelConstraints
            )
        }
    ...
    fun splineTo(pos: Vector2d, unit: Distance = Inches, tangent: Double, tangentUnit: Angle = Radians) =
        apply {
            builder = builder.splineTo(
                Vector2d(unit.of(pos.x).`in`(Inches), unit.of(pos.y).`in`(Inches)),
                tangentUnit.of(tangent).`in`(Radians),
                velConstraints,
                accelConstraints
            )
        }
    ...
    fun splineToLinearHeading(
        ...
        apply {
            builder = builder.splineToLinearHeading(
                Pose2d(
                    vectorUnit.of(pose.position.x).`in`(Inches),
                    vectorUnit.of(pose.position.y).`in`(Inches),
                    headingUnit.of(pose.heading.toDouble()).`in`(Radians)
