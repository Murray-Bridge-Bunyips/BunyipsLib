...
public class RoadRunnerTask extends Task {
    ...
    protected void periodic() {
        ...
        // Time to completion
        opMode(o -> {
            o.telemetry.add("Duration: %/% sec", round(getDeltaTime().in(Seconds), 2), round(duration, 2));
            o.telemetry.add("Distance to target: %cm", round(Centimeters.convertFrom(distance, Inches), 2));
            o.telemetry.add("Angle to target: %deg", round(Math.toDegrees(angle), 2));
        });
    }
